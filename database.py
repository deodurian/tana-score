...
def nettoyer_doublons():
    ...
    if ids_to_delete:
        ...
        for i in range(0, len(ids_to_delete), chunk_size):
            ...
            execute_query(conn, f'DELETE FROM submissions WHERE id IN ({placeholders})', tuple(chunk), commit=False)
        conn.commit()
        
    conn.close()
    return len(ids_to_delete)

def get_paginated_submissions(page=1, per_page=50):
    conn = get_db_connection()
    offset = (page - 1) * per_page
    rows = execute_query(conn, 'SELECT id, timestamp, score_t, raw_data FROM submissions ORDER BY timestamp DESC LIMIT ? OFFSET ?', (per_page, offset), fetchall=True)
    count_row = execute_query(conn, 'SELECT COUNT(*) as c FROM submissions', fetchone=True)
    conn.close()
...
def delete_submission(sub_id):
    conn = get_db_connection()
    execute_query(conn, 'DELETE FROM submissions WHERE id = ?', (sub_id,), commit=True)
    conn.close()

def get_submission_by_id(sub_id):
    conn = get_db_connection()
    row = execute_query(conn, 'SELECT * FROM submissions WHERE id = ?', (sub_id,), fetchone=True)
    conn.close()
    return row

def update_submission_raw_data(sub_id, raw_data_json):
    import TANA_code
    import json
    ...
    conn = get_db_connection()
    execute_query(conn, 'UPDATE submissions SET score_t = ?, pourcentage = ?, raw_data = ? WHERE id = ?',
                  (t_score, pourcentage, raw_data_json, sub_id), commit=True)
    conn.close()
